import{r as $,b as t,a as k,i as b}from"./shared-NCdILNy4.js";import{C as w,L as E,P as L,G as S,f as C}from"./data-DQaSi1fH.js";const q='<img src="images/house_small.png" alt="" class="filter-deco-img" aria-hidden="true">';function I(e,l){const a=e.ogImageUrl||(e.cloudinaryFolder?`https://res.cloudinary.com/ddzync8km/image/upload/${e.cloudinaryFolder}_0.jpg`:"");return e.buildingCategory,`
  <a href="property.html?id=${e.nodeId}" class="prop-card-full" 
    ...
    <div class="prop-card-img">
      ${a?`<img src="${a}" alt="${e.title}" loading="lazy" onerror="this.src='images/house_small.png';this.style.objectFit='contain';this.style.opacity='0.5';this.style.padding='20px'">`:'<div class="prop-card-img-icon"><img src="images/house_small.png" alt="" style="height:80px;opacity:0.5"></div>'}
  ...
  ${k()}
`;b();let g=[];async function x(){try{const e=await fetch(S);if(!e.ok)throw new Error("fetch failed");g=await e.json(),P(g)}catch{document.getElementById("propsGrid").innerHTML=`<div class="props-empty show">
        ${t("AlertCircle",36,1.5)}
        <p>載入失敗，請稍後再試</p>
      </div>`,document.querySelector(".props-count").textContent=""}}function P(e){const l=document.getElementById("propsGrid");l.innerHTML=e.length?e.map((s,o)=>I(s)).join("")+`<div class="props-empty" id="propsEmpty">
        ${t("Search",40,1.2)}
        <p>這個條件目前沒有物件<br>歡迎直接聯絡我詢問</p>
      </div>`:`<div class="props-empty show">${t("Search",40)}<p>目前沒有物件</p></div>`,document.querySelector(".props-count").innerHTML=`共 <strong id="propCount">${e.length}</strong> 筆物件`;const a=document.getElementById("heroStatNum");a&&(a.textContent=e.length),document.querySelectorAll(".prop-card-full").forEach((s,o)=>{s.classList.add("reveal",`reveal-d${o%3+1}`)});const i=new IntersectionObserver(s=>{s.forEach(o=>{o.isIntersecting&&o.target.classList.add("visible")})},{threshold:.1});document.querySelectorAll(".reveal").forEach(s=>i.observe(s)),A()}function A(){var l;function e(){var d,p;const a=((d=document.querySelector('input[name="category"]:checked'))==null?void 0:d.value)||"all",i=document.getElementById("filterLayout").value,s=document.getElementById("filterPrice").value,o=(((p=document.getElementById("filterSearch"))==null?void 0:p.value)||"").toLowerCase().trim(),v=document.querySelectorAll(".prop-card-full");let n=0;v.forEach(r=>{const m=a==="all"||r.dataset.category===a,y=i==="all"||r.dataset.room===i,h=s==="all"||r.dataset.price===s,f=!o||(r.dataset.search||"").includes(o),u=m&&y&&h&&f;r.classList.toggle("hidden",!u),u&&n++}),document.getElementById("propCount").textContent=n;const c=document.getElementById("propsEmpty");c&&c.classList.toggle("show",n===0)}document.querySelectorAll('input[name="category"]').forEach(a=>a.addEventListener("change",e)),document.getElementById("filterLayout").addEventListener("change",e),document.getElementById("filterPrice").addEventListener("change",e),(l=document.getElementById("filterSearch"))==null||l.addEventListener("input",e),document.querySelectorAll(".quick-tag").forEach(a=>{a.addEventListener("click",()=>{const i=document.getElementById("filterSearch"),s=a.dataset.keyword;i.value===s?(i.value="",a.classList.remove("active")):(i.value=s,document.querySelectorAll(".quick-tag").forEach(o=>o.classList.remove("active")),a.classList.add("active")),e()})})}x();
