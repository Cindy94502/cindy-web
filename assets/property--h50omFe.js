...
`;L();async function S(e){if(!e)return[];try{const s=await(await fetch(`https://res.cloudinary.com/ddzync8km/image/list/${e}.json`)).json();return!s.resources||s.resources.length===0?[]:s.resources.sort((a,r)=>(a.display_name||a.public_id).localeCompare(r.display_name||r.public_id,void 0,{numeric:!0})).map(a=>`https://res.cloudinary.com/ddzync8km/image/upload/${a.public_id}.${a.format||"jpg"}`)}catch{return[]}}function R(e){return e?Array.isArray(e)?e:(e.match(/<li>(.*?)<\/li>/g)||[]).map(t=>t.replace(/<\/?li>/g,"").trim()):[]}function h(e){const t=e.ogImageUrl||"";return`
  <a href="property.html?id=${e.nodeId}" class="similar-card">
  ...
  </a>`}function y(e,t,s){const a=document.querySelector(".similar-section");if(!a)return;const r=a.querySelector(".similar-inner"),l=Math.ceil(e.length/3);r.innerHTML=`
    ...
    `}
  `,a.style.display="",e.length>3&&b()}function M(e,t){document.querySelector(".similar-section")&&document.querySelectorAll(".clickable-tag").forEach(a=>{a.addEventListener("click",()=>{const r=a.dataset.tag,l=a.classList.contains("tag-active");if(document.querySelectorAll(".clickable-tag").forEach(n=>n.classList.remove("tag-active")),l){const n=t.filter(i=>i.nodeId!==e.nodeId&&i.title&&(i.buildingCategory===e.buildingCategory||i.priceRange===e.priceRange)).slice(0,9).map(i=>({...i,reason:i.buildingCategory===e.buildingCategory&&i.priceRange===e.priceRange?`同為${i.buildingCategory}・${i.priceRange}`:i.buildingCategory===e.buildingCategory?`同為${i.buildingCategory}`:`同價格區間・${i.priceRange}`}));y(n,"也許你也會喜歡","相似物件")}else{a.classList.add("tag-active");const n=t.filter(i=>i.nodeId!==e.nodeId&&i.title&&((i.tags||"").split(",").map(o=>o.trim()).includes(r)||(i.wixParking||"").trim()===r||(i.wixLocation||"").includes(r)||r.includes(i.wixLocation||""))).slice(0,9).map(i=>({...i,reason:`同樣有「${r}」`}));y(n,`選擇了「${r}」`,"相似物件")}})})}async function B(){try{const t=await(await fetch(I)).json(),s=t.find(r=>r.nodeId===E);if(!s){document.querySelector(".property-loading").innerHTML='<p>找不到這個物件，<a href="properties.html" style="color:var(--teal-dark)">回到物件列表</a></p>';return}document.title=`${s.title} | Cindy 小薰`;const a=await S(s.cloudinaryFolder);s.ogImageUrl&&!a.includes(s.ogImageUrl)&&a.unshift(s.ogImageUrl);try{const r=await fetch("/cindy-web/market-data.json");r.ok&&(v=(await r.json())[s.nodeId]||null)}catch{v=null}q(s,a,t)}catch{document.querySelector(".property-loading").innerHTML="<p>載入失敗，請稍後再試</p>"}}let v=null;function P(e){return""}function A(){const e=document.querySelector(".market-help"),t=e==null?void 0:e.querySelector(".market-tip");if(!e||!t)return;const s=()=>{t.style.visibility="hidden",t.style.display="block";const a=e.getBoundingClientRect(),r=Math.min(430,window.innerWidth*.92);let l=a.right-r;l=Math.max(10,Math.min(l,window.innerWidth-r-10)),t.style.left=l+"px",t.style.transform="none",t.style.top=Math.max(10,a.top-t.offsetHeight-10)+"px",t.style.display="",t.style.visibility=""};e.addEventListener("mouseenter",s),e.addEventListener("focus",s)}function T(){var l,n;const e=document.getElementById("marketBody");if(!e)return;const t=[...e.querySelectorAll("tr")],s=Math.ceil(t.length/5);if(s<=1)return;let a=0;const r=()=>{t.forEach(i=>{i.style.display=i.dataset.mpage==a?"":"none"}),document.getElementById("marketPageInfo").textContent=`${a+1} / ${s}`,document.getElementById("marketPrev").disabled=a===0,document.getElementById("marketNext").disabled=a===s-1};(l=document.getElementById("marketPrev"))==null||l.addEventListener("click",()=>{a>0&&(a--,r())}),(n=document.getElementById("marketNext"))==null||n.addEventListener("click",()=>{a<s-1&&(a++,r())}),r()}function j(e){return e.length===0?`<div class="gallery-placeholder">${c("Home",80,1,"","var(--sage-dark)")}</div>`:`
  ...
  `}function U(e){var l,n;if(e.length===0)return;let t=0;const s=document.getElementById("galleryMain"),a=document.querySelectorAll(".gallery-thumb");function r(i){var o;t=(i+e.length)%e.length,s.style.opacity="0",setTimeout(()=>{s.src=e[t],s.style.opacity="1"},150),a.forEach((d,m)=>d.classList.toggle("active",m===t)),(o=a[t])==null||o.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})}(l=document.getElementById("galleryPrev"))==null||l.addEventListener("click",()=>r(t-1)),(n=document.getElementById("galleryNext"))==null||n.addEventListener("click",()=>r(t+1)),a.forEach((i,o)=>i.addEventListener("click",()=>r(o))),document.addEventListener("keydown",i=>{i.key==="ArrowLeft"&&r(t-1),i.key==="ArrowRight"&&r(t+1)})}function q(e,t,s=[]){const a=R(e.wixFeatures),r=(e.tags||"").split(",").filter(Boolean),l=s.filter(n=>n.nodeId!==e.nodeId&&n.title&&(n.buildingCategory===e.buildingCategory||n.priceRange===e.priceRange)).slice(0,9).map(n=>({...n,reason:n.buildingCategory===e.buildingCategory&&n.priceRange===e.priceRange?`同為${n.buildingCategory}・${n.priceRange}`:n.buildingCategory===e.buildingCategory?`同為${n.buildingCategory}`:`同價格區間・${n.priceRange}`}));document.querySelector(".property-page").innerHTML=`
  ...
  `,_(e,t),U(t),z(e),T(),A(),M(e,s),b()}function b(){const e=document.getElementById("similarGrid"),t=document.getElementById("similarPrev"),s=document.getElementById("similarNext"),a=document.querySelectorAll(".similar-dot");if(!e||!t||!s)return;const r=e.querySelectorAll(".similar-card"),l=r.length,n=window.innerWidth<=768?2:3,i=Math.ceil(l/n);let o=0;function d(m){o=Math.max(0,Math.min(m,i-1));const g=parseInt(getComputedStyle(e).columnGap)||40,p=r[0].offsetWidth,$=n*(p+g);e.style.transform=`translateX(-${o*$}px)`,a.forEach((f,w)=>f.classList.toggle("active",w===o)),t.style.opacity=o===0?"0.3":"1",s.style.opacity=o===i-1?"0.3":"1"}t.addEventListener("click",()=>{o>0&&d(o-1)}),s.addEventListener("click",()=>{o<i-1&&d(o+1)}),a.forEach((m,g)=>m.addEventListener("click",()=>d(g))),d(0)}function _(e,t){const s=location.href,a=e.ogImageUrl||t[0]||"",r=(l,n)=>{let i=document.querySelector(`meta[property="${l}"]`);i||(i=document.createElement("meta"),i.setAttribute("property",l),document.head.appendChild(i)),i.setAttribute("content",n)};r("og:title",`${e.title} | Cindy 小薰`),r("og:description",`💰 售價與詳情請點擊查閱。格局：${e.layout||"—"}，${e.wixLocation||""}精選房源推薦。`),r("og:image",a),r("og:url",s),r("og:type","website")}function z(e){var o,d,m;const t=`https://cindy94502.github.io/cindy-web/p/${encodeURIComponent(e.nodeId)}.html`,s=[e.layout?`格局：${e.layout}`:"",e.buildingSize?`建坪：${e.buildingSize}坪`:"",e.landSize?`土坪：${e.landSize}坪`:""].filter(Boolean).join(`
...
`+n)}`,"_blank")}),(m=document.getElementById("btnShareFb"))==null||m.addEventListener("click",()=>{const g=`fb-messenger://share?link=${encodeURIComponent(t)}`;window.location.href=g,setTimeout(()=>{window.open(`https://www.facebook.com/dialog/send?link=${encodeURIComponent(t)}&app_id=291494419107518&redirect_uri=${encodeURIComponent(t)}`,"_blank")},1500)})}B();
