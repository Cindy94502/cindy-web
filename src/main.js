...
// 只有一邊會下載，src 都是 JS 才填的。
{
  ...
  if (isDesktop && heroSection && scrubWrap) {
    ...
    // 40 張 1280x720 WebP q86，共 2781 KB。
    // 調整過兩次：960寬 q72（1099KB）太糊 → 1280寬 q74（1707KB）還是有點糊
    // → 1280寬 q86。把四種設定都放大到 2880（Retina 上 1440 CSS px 的實際
    // 像素）比對過，q86 的髮絲和睫毛已經很接近原始 PNG，剩下的差距是
    // 來源本身只有 1280x720 的天花板，再加碼沒有意義。
    const FRAMES = 40
    const imgs = []
