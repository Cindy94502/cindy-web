...
`;L();async function E(e){if(!e)return[];try{const s=await(await fetch(`https://res.cloudinary.com/ddzync8km/image/list/${e}.json`)).json();return!s.resources||s.resources.length===0?[]:s.resources.sort((n,r)=>(n.display_name||n.public_id).localeCompare(r.display_name||r.public_id,void 0,{numeric:!0})).map(n=>`https://res.cloudinary.com/ddzync8km/image/upload/${n.public_id}.${n.format||"jpg"}`)}catch{return[]}}function R(e){return e?Array.isArray(e)?e:(e.match(/<li>(.*?)<\/li>/g)||[]).map(a=>a.replace(/<\/?li>/g,"").trim()):[]}function y(e){const a=e.ogImageUrl||"";return`
  ...
  `,U(e,a),T(a),q(e),S(e,s),p()}function p(){const e=document.getElementById("similarGrid"),a=document.getElementById("similarPrev"),s=document.getElementById("similarNext"),n=document.querySelectorAll(".similar-dot");if(!e||!a||!s)return;const r=e.querySelectorAll(".similar-card"),l=r.length,t=window.innerWidth<=768?2:3,i=Math.ceil(l/t);let o=0;function d(m){o=Math.max(0,Math.min(m,i-1));const g=parseInt(getComputedStyle(e).columnGap)||40,b=r[0].offsetWidth,h=t*(b+g);e.style.transform=`translateX(-${o*h}px)`,n.forEach(($,f)=>$.classList.toggle("active",f===o)),a.style.opacity=o===0?"0.3":"1",s.style.opacity=o===i-1?"0.3":"1"}a.addEventListener("click",()=>{o>0&&d(o-1)}),s.addEventListener("click",()=>{o<i-1&&d(o+1)}),n.forEach((m,g)=>m.addEventListener("click",()=>d(g))),d(0)}function U(e,a){const s=location.href,n=e.ogImageUrl||a[0]||"",r=(l,t)=>{let i=document.querySelector(`meta[property="${l}"]`);i||(i=document.createElement("meta"),i.setAttribute("property",l),document.head.appendChild(i)),i.setAttribute("content",t)};r("og:title",`${e.title} | Cindy 王小姐`),r("og:description",`💰 售價與詳情請點擊查閱。格局：${e.layout||"—"}，${e.wixLocation||""}精選房源推薦。`),r("og:image",n),r("og:url",s),r("og:type","website")}function q(e){var n,r,l;const a=`https://cindy94502.github.io/cindy-web/p/${encodeURIComponent(e.nodeId)}.html`,s=`${e.title}｜NT${e.price?(e.price/1e4).toFixed(0)+"萬":"洽談"}｜Cindy 王小姐`;(n=document.getElementById("btnCopyLink"))==null||n.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(a);const t=document.getElementById("btnCopyLink");t.textContent="✓ 已複製！",setTimeout(()=>t.textContent="複製連結",2e3)}catch{alert("請手動複製網址")}}),(r=document.getElementById("btnShareLine"))==null||r.addEventListener("click",()=>{window.open(`https://line.me/R/msg/text/?${encodeURIComponent(s+`
