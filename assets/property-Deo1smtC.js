...
`;L();async function E(e){if(!e)return[];try{const s=await(await fetch(`https://res.cloudinary.com/ddzync8km/image/list/${e}.json`)).json();return!s.resources||s.resources.length===0?[]:s.resources.sort((r,n)=>(r.display_name||r.public_id).localeCompare(n.display_name||n.public_id,void 0,{numeric:!0})).map(r=>`https://res.cloudinary.com/ddzync8km/image/upload/${r.public_id}.${r.format||"jpg"}`)}catch{return[]}}function S(e){return e?Array.isArray(e)?e:(e.match(/<li>(.*?)<\/li>/g)||[]).map(a=>a.replace(/<\/?li>/g,"").trim()):[]}function y(e){const a=e.ogImageUrl||"";return`
  ...
  `,U(e,a),T(a),q(e),R(e,s),b()}function b(){const e=document.getElementById("similarGrid"),a=document.getElementById("similarPrev"),s=document.getElementById("similarNext"),r=document.querySelectorAll(".similar-dot");if(!e||!a||!s)return;const n=e.querySelectorAll(".similar-card"),l=n.length,t=window.innerWidth<=768?2:3,i=Math.ceil(l/t);let o=0;function d(m){o=Math.max(0,Math.min(m,i-1));const g=parseInt(getComputedStyle(e).columnGap)||40,h=n[0].offsetWidth,p=t*(h+g);e.style.transform=`translateX(-${o*p}px)`,r.forEach(($,f)=>$.classList.toggle("active",f===o)),a.style.opacity=o===0?"0.3":"1",s.style.opacity=o===i-1?"0.3":"1"}a.addEventListener("click",()=>{o>0&&d(o-1)}),s.addEventListener("click",()=>{o<i-1&&d(o+1)}),r.forEach((m,g)=>m.addEventListener("click",()=>d(g))),d(0)}function U(e,a){const s=location.href,r=e.ogImageUrl||a[0]||"",n=(l,t)=>{let i=document.querySelector(`meta[property="${l}"]`);i||(i=document.createElement("meta"),i.setAttribute("property",l),document.head.appendChild(i)),i.setAttribute("content",t)};n("og:title",`${e.title} | Cindy 王小姐`),n("og:description",`💰 售價與詳情請點擊查閱。格局：${e.layout||"—"}，${e.wixLocation||""}精選房源推薦。`),n("og:image",r),n("og:url",s),n("og:type","website")}function q(e){var n,l,t;const a=`https://cindy94502.github.io/cindy-web/p/${encodeURIComponent(e.nodeId)}.html`,s=[e.layout?`格局：${e.layout}`:"",e.buildingSize?`建坪：${e.buildingSize}坪`:"",e.landSize?`土坪：${e.landSize}坪`:""].filter(Boolean).join(`
`),r=`${e.title}｜NT${e.price?(e.price/1e4).toFixed(0)+"萬":"洽談"}｜Cindy 王小姐`+(s?`
`+s:"");(n=document.getElementById("btnCopyLink"))==null||n.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(a);const i=document.getElementById("btnCopyLink");i.textContent="✓ 已複製！",setTimeout(()=>i.textContent="複製連結",2e3)}catch{alert("請手動複製網址")}}),(l=document.getElementById("btnShareLine"))==null||l.addEventListener("click",()=>{window.open(`https://line.me/R/msg/text/?${encodeURIComponent(r+`
