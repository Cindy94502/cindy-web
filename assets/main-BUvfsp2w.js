import{r as B,b as a,a as C,i as L}from"./shared-C92FQ_gJ.js";import{G as A,c as x,f as I}from"./data-vfwjmF8_.js";function f(t,i){return`<div class="torn-divider" style="background:${i}">
  ...
  </div>`}function H(t,i){const r=x(t.ogImageUrl,600),v=t.buildingCategory==="透天"?"House":"Building2";return`
  <a href="property.html?id=${t.nodeId}" class="prop-card reveal reveal-d${i+1}">
    <div class="prop-card-img">
      ${r?`<img src="${r}" alt="${t.title}" loading="lazy" style="width:100%;height:100%;object-fit:cover;position:absolute;inset:0" onerror="this.style.objectFit='contain';this.style.opacity='0.4';this.style.padding='20px';this.src='images/house_small.png'">`:`<div class="prop-card-img-icon">${a(v,48,1,"prop-placeholder-icon")}</div>`}
  ...
  ${C()}
`;L();setTimeout(()=>{var t;(t=document.getElementById("heroStrong"))==null||t.classList.add("hero-title-line-animate")},1400);var w;(w=document.getElementById("scrollHint"))==null||w.addEventListener("click",t=>{t.preventDefault();const i=document.getElementById("about");i&&window.scrollTo({top:i.offsetTop-64,behavior:"smooth"})});{const t=document.getElementById("hero"),i=document.getElementById("heroScrub"),r=document.getElementById("heroLoopVideo"),v=matchMedia("(min-width: 901px) and (hover: hover) and (pointer: fine)").matches,c=matchMedia("(prefers-reduced-motion: reduce)").matches;if(v&&t&&i){const d=[];for(let e=0;e<40;e++){const s=new Image;s.decoding="async",s.alt="",s.className="hero-frame-img",i.appendChild(s),d.push(s)}const E=()=>{const e=[],s=new Set;for(const o of[8,4,2,1])for(let n=0;n<40;n+=o)s.has(n)||(s.add(n),e.push(n));return e},m=new Array(40).fill(!1);let p=-1;const u=e=>{if(e=Math.max(0,Math.min(39,Math.round(e))),!m[e]){let s=-1;for(let o=1;o<40;o++){if(e-o>=0&&m[e-o]){s=e-o;break}if(e+o<40&&m[e+o]){s=e+o;break}}if(s<0)return;e=s}e!==p&&(d[e].style.zIndex="2",d[e].classList.add("on"),p>=0&&(d[p].style.zIndex="1",d[p].classList.remove("on")),p=e)},b=()=>{for(const e of E())d[e].addEventListener("load",()=>{const s=()=>{m[e]=!0,p<0&&u(e)};d[e].decode?d[e].decode().then(s,s):s()},{once:!0}),d[e].src=`media/frames/${String(e).padStart(2,"0")}.webp`};document.readyState==="complete"?setTimeout(b,200):window.addEventListener("load",()=>setTimeout(b,200));const $=37/720,M=64,F=960,g=540,y=()=>{const e=i.clientWidth,s=i.clientHeight;if(!e||!s)return;const o=Math.max(e/F,s/g),n=g*o-s,h=n<=0?0:Math.max(0,Math.min(100,(g*$*o-M)/n*100));i.style.setProperty("--hero-vpos",h.toFixed(1)+"%")};if(y(),window.addEventListener("resize",y),!c){let s=0,o=0,n=!1;const h=()=>{o+=(s-o)*.18,u(o),Math.abs(s-o)>.4?requestAnimationFrame(h):n=!1};t.addEventListener("mousemove",S=>{s=(1-S.clientX/window.innerWidth)*39,n||(n=!0,requestAnimationFrame(h))})}}else if(r&&!c){const l=()=>{r.autoplay=!0,r.src="media/hero-loop.mp4",r.addEventListener("loadeddata",()=>r.play().catch(()=>{}),{once:!0}),r.play().catch(()=>{})};document.readyState==="complete"?setTimeout(l,300):window.addEventListener("load",()=>setTimeout(l,300))}}async function R(){try{const i=await(await fetch(A)).json(),r=i.filter(c=>c.title&&c.nodeId).slice(0,3);document.getElementById("homePropsGrid").innerHTML=r.map((c,l)=>H(c,l)).join(""),document.getElementById("propsMoreBtn").innerHTML=`查看全部 ${i.length} 筆物件 ${a("ArrowRight",16,2)}`;const v=new IntersectionObserver(c=>{c.forEach(l=>{l.isIntersecting&&l.target.classList.add("visible")})},{threshold:.1});document.querySelectorAll("#homePropsGrid .reveal").forEach(c=>v.observe(c))}catch{document.getElementById("homePropsGrid").innerHTML='<div style="grid-column:1/-1;text-align:center;padding:40px;color:var(--brown-mid)">暫時無法載入物件</div>'}}R();
