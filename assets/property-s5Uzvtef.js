import{r as L,a as x,i as I,b as c}from"./shared-C05YdUy_.js";import{G as k,f as v}from"./data-DQaSi1fH.js";const E=new URLSearchParams(location.search),R=E.get("id");document.getElementById("app").innerHTML=`
  ${L()}
  <div class="property-page">
    <div class="property-loading">載入中...</div>
  </div>
  ${x()}
`;I();async function S(e){if(!e)return[];try{const s=await(await fetch(`https://res.cloudinary.com/ddzync8km/image/list/${e}.json`)).json();return!s.resources||s.resources.length===0?[]:s.resources.sort((n,r)=>(n.display_name||n.public_id).localeCompare(r.display_name||r.public_id,void 0,{numeric:!0})).map(n=>`https://res.cloudinary.com/ddzync8km/image/upload/${n.public_id}.${n.format||"jpg"}`)}catch{return[]}}function M(e){return e?(e.match(/<li>(.*?)<\/li>/g)||[]).map(a=>a.replace(/<\/?li>/g,"").trim()):[]}function y(e){const a=e.ogImageUrl||"";return`
